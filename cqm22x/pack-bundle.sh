...
pack-bundle.sh --version V [options]
  ...
  --map REL=DIR      take REL from DIR instead of \$SOURCE/REL (repeatable)
                     openwrt-host almost always needs one, e.g.
                     --map openwrt-host=/pkg/openwrt/staging_dir/host
...
#
# openwrt-prebuilt-backup is not a Qualcomm toolchain: it is OpenWrt's own
# host tools and cross toolchain, captured after a full app build.
# set_openwrt_env.sh restores it automatically (prepare_toolchain ->
# prebuilt_cache_restore --relocate), which turns the first app build from a
# from-scratch gcc/binutils/musl build into an extract.
#
# It bakes the absolute checkout path into its binaries and can only be
# relocated to a path no longer than the original — which is precisely why it
# is shippable here and was not before: inside the container the tree is always
# under /work, whatever the host path happens to be.
INCLUDE=(
    ...
    sectools
    prebuilts
    openwrt-prebuilt-backup
)

# The cache directory often holds both foo.tar and foo.tar.zst — the same
